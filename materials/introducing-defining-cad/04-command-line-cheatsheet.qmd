...
If you are using WSL (Windows Subsystem for Linux), macOS, or Linux, you should use the macOS/Linux/WSL cheatsheet. If you are using Windows PowerShell, you should use the Windows PowerShell cheatsheet. Remember, you can use either kind of terminal inside VS Code, so always check which terminal profile you are using.
...
| `open [file]` | Opens a file on macOS |
| `open .` | Opens the current directory on macOS |
| `xdg-open [file]` | Opens a file on many Linux desktop systems |
| `xdg-open .` | Opens the current directory on many Linux desktop systems |
| `explorer.exe [file]` | Opens a file from WSL using Windows |
| `explorer.exe .` | Opens the current WSL directory using Windows File Explorer |
...
| `cp *.js [directory]`| Use wildcards to get all files of a certain type when moving or copying|
| `!!` | Use double bang to repeat last command |
| `nano [file]` | Opens file in Terminal editor |
| `q` | Exit many terminal programs, such as `less`, `man`, or `git log` |
