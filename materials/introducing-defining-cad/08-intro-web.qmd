...
This diagram is from the Mozilla Web Docs, which provide a great overview of the topic [https://developer.mozilla.org/en-US/docs/Learn_web_development/Howto/Web_mechanics/What_is_a_URL](https://developer.mozilla.org/en-US/docs/Learn_web_development/Howto/Web_mechanics/What_is_a_URL){target="_blank"}. You've already encountered URLs when we created an `a` tag in our HTML page.

```html
<a target="_blank" href="https://ischool.illinois.edu/">My first page!</a>
```

This is a type of **hyperlink** or **link** that allows us to connect webpages. Mozilla describes three main types of links. An **internal link** connects two pages that belong to the same website. An **external link** points from your webpage to someone else's webpage. An **incoming link** points from someone else's webpage back to your site. In our example, we created an external link because we linked to a webpage that is not part of our website. You can also use page fragments or anchors to link to a specific section within a webpage, like the sidebar navigation on this course site. You can read more about hyperlinks here [https://developer.mozilla.org/en-US/docs/Learn_web_development/Howto/Web_mechanics/What_are_hyperlinks#types_of_links](https://developer.mozilla.org/en-US/docs/Learn_web_development/Howto/Web_mechanics/What_are_hyperlinks#types_of_links){target="_blank"}.
...
While this is a simplified explanation, computers can act as **clients** or **servers** depending on the role they play. A client is a computer that requests data from a server, and a server is a computer that provides data to a client in response.
...
Now we can look more closely at the components of URL, through the Mozilla example here [https://developer.mozilla.org/en-US/docs/Learn/Common_questions/Web_mechanics/What_is_a_URL#scheme](https://developer.mozilla.org/en-US/docs/Learn/Common_questions/Web_mechanics/What_is_a_URL#scheme){target="_blank"}.
...
- a *website* is a collection of web pages and related content that is identified by a common **domain name** and published on at least one web server, like our course website. A website uses internal links to connect its own pages, external links to point outward to other websites, and may receive incoming links from other sites.
...
For example, the 1995 classic, thriller movie [*The Net*](https://en.wikipedia.org/wiki/The_Net_(1995_film)){target="_blank"} starring Sandra Bullock as a systems analyst who "must use her computer skills to uncover the truth and clear her name." In the movie, she searches using IP addresses which you can watch a clip of here: [https://youtu.be/M1K-B8QVLyo?t=242](https://youtu.be/M1K-B8QVLyo?t=242){target="_blank"}.

IP Addresses are still used today, but they are not very human readable or user friendly (though we will be seeing some in action today), so instead, we use domain names. Domain names are part of the [**Domain Name System (DNS)**](https://en.wikipedia.org/wiki/Domain_Name_System){target="_blank"} which is often described as a "phone book" or "address book" that translates IP addresses of servers into easier to understand domains.
...
Make sure:
- Use a **Public** repository unless you know your account or organization supports private GitHub Pages
...
Once you push, GitHub usually detects the change and starts hosting your site automatically. If it does not, check **Settings** → **Pages**.
...
Create a folder for your digital object (e.g., `digital-objects/member-name/`) and add your `index.html` file there.

::: {.callout-important}
### Your Group Repository Needs a Root `index.html`

Your group must also create an `index.html` in the **root of the repository**. This file is the homepage GitHub Pages displays at your group's main website address. It should identify your group and link to each member's page using relative links such as:

```html
<a href="./digital-objects/member-name/">Member's digital object</a>
```

Without a root `index.html`, individual pages may still exist at their full URLs, but the group's main website address will not have a homepage.
:::

Example structure:
```
is310-fall-2026-group-*/
├── index.html (group homepage with links to members)
└── digital-objects/
    ├── member1-name/
    │   └── index.html
    ├── member2-name/
...
Your group's homepage will be hosted at: `https://cultureasdata-uiuc.github.io/is310-fall-2026-group-*/`

Individual project pages will use their folder paths, such as: `https://cultureasdata-uiuc.github.io/is310-fall-2026-group-*/digital-objects/member-name/`
...
Example of a deployed personal site: [https://testzoe.github.io/](https://testzoe.github.io/){target="_blank"}
...
Now that you understand how the web works and how GitHub Pages works, it's time to deploy your own website! You'll take the HTML page you created in the [Source & Style Assignment](../introducing-defining-cad/07-intro-html.qmd#homework-source-and-style){target="_blank"} and host it online.
...
If you use an AI chatbot to help with this assignment, include an `ai-chat-log.md` file that records the relevant prompts and responses from that conversation.
...
3. Post the link to your live site in the GitHub Discussion thread [https://github.com/CultureAsData-UIUC/is310-fall-2026/discussions/4](https://github.com/CultureAsData-UIUC/is310-fall-2026/discussions/4){target="_blank"}


[^1]: Interestingly, the Product Management/Note Taking app *Notion* has a great interview with Nelson for those that are interested in his work [https://www.notion.so/blog/ted-nelson](https://www.notion.so/blog/ted-nelson){target="_blank"}.
