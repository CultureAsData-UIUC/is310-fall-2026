...
Let's take a look at a possible code answer for the final quick exercise in [Python Refresher II: Advanced Python](../creating-curating-cad/02-python-refresher-advanced){target="_blank"}.
...
This code snippet contains a **function** called `check_movie_release` that returns a movie name if it was released after a certain year.
...
To first define a class, we need to use the keyword `class` followed by the name of the class and then a colon. Class names conventionally use `CapitalizedWords`. All the logic of the class is indented (just like with functions, loops, and conditionals), and then we call the class similarly to a function, using its name and parentheses to pass arguments to the class.
...
`self` references the class **instance** that is created when you call a class. By convention, it is the first parameter of an **instance method**. Python also supports class methods and static methods, which work differently, but we will focus on instance methods here. You can read more about [class scope here](https://docs.python.org/3/tutorial/classes.html#python-scopes-and-namespaces){target="_blank"}.
...
This class serves a purpose similar to a dictionary because both can group related values. The syntax differs, however: dictionaries access values with keys, while class instances access attributes with dot notation.
...
What if we wanted to add a method for printing out an entire movie's information? We could try looping through the class object and print out each attribute. Or we could use the built-in `__dict__` attribute, which stores an instance's attributes in a dictionary. Add this method inside the `Movie` class at the same indentation level as the other methods:
...
This is a more complex version of our `Movie` class. We've added two new methods, `add_sequel` and `add_prequel`, which take the name and release year of another movie. We then create a new instance of the `Movie` class and append it to the `sequels` or `prequels` list. We also added `calculate_movie_order`, which sorts all the movies by release year and returns their names in order. The built-in `sort` method sorts the list in place. Its `key` argument accepts a function; here, a short `lambda` function tells Python to use each movie's `release_year` as the sorting value.
...
1. [An Introduction to Python Classes and Inheritance](http://www.jesshamrick.com/2011/05/18/an-introduction-to-classes-and-inheritance-in-python/){target="_blank"}
2. [Here is a very helpful video series on class inheritance](https://www.youtube.com/playlist?list=PL-osiE80TeTsqhIuOqKhwlXsIBIdSeYtc){target="_blank"}
3. [Python Documentation on Classes](https://docs.python.org/3/tutorial/classes.html){target="_blank"}
...
For longer explanations, Python programmers usually place several `#` comments on consecutive lines. Functions and classes can also contain **docstrings**, which document how that code should be used.
...
In this example, the triple-quoted string is the first statement inside the function, so Python treats it as the function's **docstring**. Triple-quoted strings are not general-purpose comments: when they appear elsewhere, they are simply multiline string values.

```python
print(a_movie.add_directors.__doc__) # To view the docstring for the method
```

Earlier, we were able to print the `add_directors` documentation because it is a **docstring**. A docstring is the first statement in a function, method, class, or module and is conventionally enclosed in triple quotes. Adding concise docstrings to reusable functions and classes helps other people understand the code and helps you remember how to use it later.
...
So for example, if we wanted to learn more about classes, we could visit the [Python Documentation on Classes](https://docs.python.org/3/tutorial/classes.html){target="_blank"}. This documentation explains how to define a class, how to create an instance of a class, and how to use methods in a class. It also explains how to use inheritance, which is a way to create a new class that inherits the attributes and methods of an existing class.
...
[Here's the Python Standard Library](https://docs.python.org/3/library/){target="_blank"}. We've already been using this documentation, but let's scroll down to the [Pathlib module](https://docs.python.org/3/library/pathlib.html){target="_blank"}.
...
First take a look at the current [`pathlib` source code](https://github.com/python/cpython/blob/main/Lib/pathlib/__init__.py){target="_blank"}. Notice how it's organized, and try to find the definition of `class Path` (*hint*: use Control+F or Command+F).
...
Both options give us access to the same `Path` class, but they create different names in our script. With `import pathlib`, we write `pathlib.Path()`. With `from pathlib import Path`, we write the more concise `Path()`.
...
Depending on where your file is located, you might need to change the path. `pathlib` lets us construct paths that work across macOS, Linux, and Windows. For example, if the file is in your Downloads folder, use:

```python
file = Path.home() / "Downloads" / "US-National-Parks_RecreationVisits_1979-2023.csv"
```

Computers need the **exact path**, so if you're unsure where Python is looking, use `Path.cwd()` to print the current working directory.
...
At the end, we close the file so that system resources are released and any pending changes are written. Forgetting to close a file can produce unreliable behavior, so Python's `with` syntax is usually the safer choice.
...
This is functionally similar to our last example, but the file is automatically closed at the end of the block, even if an error occurs. You will see both conventions, but prefer `with` for your own code.

### File Output

File output is very similar. We just use mode `w`, which overwrites the file specified. We can also use `x` (which only works for new files) or `a` (which appends data at the end of the file). Read the [`open` docs](https://docs.python.org/3/library/functions.html#open){target="_blank"} for all the details and also take a look at this table.
...
For the same reason that we can't do `output_file.write(4)`, we also can't do `"4" + 4` (or, for that matter, `4 + "4"`). We have to explicitly tell Python whether we want it to treat each value as an integer or a string. `"4" + str(4)` produces `"44"`, while `int("4") + 4` returns `8`.
...
::: {.callout-tip}
## Prefer Structured Data for Real Projects

The text format in this exercise is useful for practicing file input and output, but splitting lines on punctuation can break when values contain unexpected characters or fields are missing. For a real dataset, prefer a structured format such as JSON and use Python's built-in [`json` module](https://docs.python.org/3/library/json.html){target="_blank"} to read and write it.
:::
