...
The easiest way to work with Python is through the command line. Let's start by first checking our Python version. Open your terminal (whether in VS Code or your preferred terminal app) and type:
...
If you don't see these messages, then you might not have Python installed, so return to our [setup instructions](../introducing-defining-cad/01-CAD-tools/#python){target="_blank"} and reach out to the instructors for assistance.

In this course, you should be using some version of Python 3. If you're using Python 2, you should upgrade to Python 3. Python 2 is no longer supported. Historically, `python` was used to call Python 2 and `python3` was used to call Python 3. However, because Python 2 is no longer supported, `python` now usually calls Python 3 on current systems. You can read more at the [Python 3 documentation](https://docs.python.org/3/){target="_blank"} for more information.

----

Once you have Python working, you can type:

```sh
python
```

This will open the interactive Python interpreter where you can write your python code and have the computer execute it. This Python interpreter is a type of program that reads and executes Python code (i.e. either Python syntax or files that end with `.py`). The interactive Python interpreter is an excellent way to sandbox your ideas or check your syntax. You can exit using `exit()`.

Type `"Hello world"` in the interpreter and press enter. Make sure to use single or double quotes around this text (otherwise you'll see a `SyntaxError`, something we'll discuss later in this lesson).
...
You'll notice that unlike with `"Hello World"` the interpreter doesn't just show our data back to us. Instead, it displays the result of our calculations. This is because the interpreter is designed to show us the value of each expression we enter. In our first example, we entered a string expression. In our second example, we entered mathematical expressions, so the interpreter showed us the results of those calculations.
...
- A variable can't have a **reserved word** as a name. The creators and developers of Python have selected a series of words that have a special meaning and function when you are programming in Python. If you try to use these words, then your computer will get confused. For example, if you try to use `and` as a variable name, you'll get a Syntax Error again . You can see a list of reserved words [here](https://www.w3schools.com/python/python_ref_keywords.asp){target="_blank"}, as well as in the figure below. Don't worry about memorizing these, but it's helpful to know that they exist.
...
For some more examples, see Melanie Walsh's note on ["Striving for Good Variable Names"](https://melaniewalsh.github.io/Intro-Cultural-Analytics/02-Python/04-Variables.html#striving-for-good-variable-names){target="_blank"}.
...
For more information check out [Float to Binary Converter](https://www.h-schmidt.net/FloatConverter/IEEE754.html){target="_blank"} and [IEEE 754 Standard](https://en.wikipedia.org/wiki/IEEE_754-1985){target="_blank"}.
...
This type of calculation uses an **operator** in Python. Operators are special symbols in Python that carry out arithmetic, comparison, or logical computation. The value that the operator operates on is called the **operand**.

For addition, we use the `+` operator symbol.
...
You should see `TypeError: unsupported operand type(s) for +: 'int' and 'str'`. This is because Python can't add an integer to a string.
...
You can find more information about operators here [https://www.w3schools.com/python/python_operators.asp](https://www.w3schools.com/python/python_operators.asp){target="_blank"}.
...
Notice to do concatenation we use the `+` operator. This would return a new list that contains all of our favorite and least favorite movies. We could then store this new list in a variable called our `movie_collection`.
...
Now each of our movies is a dictionary that contains the name and release year of our movie. We can access the value for each movie using either `movie_1` or `movie_2`.
...
This combining of lists and dictionaries also means that we can put dictionaries inside of lists:
...
Because these dictionaries are inside a list, Python indexes each list item with numbers, just like in our list of strings. So to get the first value, you would type:
