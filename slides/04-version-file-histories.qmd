...
::: {.notes}
The `-c` flag with grep counts the number of matching lines instead of displaying them. This is useful for quick text analysis, but it is not quite the same thing as counting every individual occurrence of a word, since a line could include the same word more than once.
:::
